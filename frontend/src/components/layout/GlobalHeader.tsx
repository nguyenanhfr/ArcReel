...
/** bằng cách ẩn <a> Kích hoạt tải xuống trình duyệt để tránh window.open tạo các tab trống */
...
const PHASES = [
  ...
  { key: "scripting", label: "Sáng tác kịch bản" },
...
export function GlobalHeader({ onNavigateBack }: GlobalHeaderProps) {
  ...
  // Tải thống kê chi phí (tự động làm mới khi hoàn thành nhiệm vụ)
  ...
  const modeBadgeText =
    contentMode === "drama" ? "Hoạt hình phim 16:9" : "Chế độ kể chuyện 9:16";
  ...
  const handleJianyingExport = async (episode: number, draftPath: string, jianyingVersion: string) => {
    ...
    try {
      ...
      useAppStore.getState().pushToast("Đã bắt đầu xuất bản nháp Jianying, vui lòng giải nén ZIP tải về vào thư mục bản nháp Jianying", "success");
    } catch (err) {
    ...
    try {
      ...
      if (diagnosticCount > 0) {
        ...
        useAppStore.getState().pushToast(
          `Đã bắt đầu tải xuống ZIP dự án，Gói xuất khẩu chứa ${diagnosticCount} Chẩn đoán`,
          "warning",
        );
      } else {
        useAppStore.getState().pushToast("Đã bắt đầu tải xuống ZIP dự án", "success");
      }
    } catch (err) {
      useAppStore
        .getState()
    ...
    <header className="flex h-12 shrink-0 items-center justify-between border-b border-gray-800 bg-gray-900/80 px-4 backdrop-blur-sm">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={taskHudAnchorRef}>
          <button
            ...
            title={`Trạng thái tác vụ: ${stats.running} Đang chạy, ${stats.queued} Đang xếp hàng`}
        ...
        <div className="relative" ref={exportAnchorRef}>
          <button
            ...
            title="Xuất dự án ZIP hiện tại"
            aria-label="Xuất dự án ZIP hiện tại"
          >
            {exportingProject ? (
      ...
      {exportDiagnostics !== null && (
        <ArchiveDiagnosticsDialog
          ...
          description="Xuất đã hoàn tất kiểm tra trước và tạo ZIP. Các vấn đề sau đây được phát hiện trong gói xuất."
          sections={[
