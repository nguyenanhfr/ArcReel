    ...
    >
      <div className="flex items-center gap-3">
        <button
          ...
          aria-label={cred.is_active ? "Hiện tạiĐang sử dụng" : `Kích hoạt ${cred.name}`}
        ...
        <div className="flex flex-shrink-0 items-center gap-1">
          <button
            ...
            onClick={handleTest}
            disabled={testing}
            aria-label={`Kiểm tra ${cred.name} kết nối`}
        ...
        >
          ...
          {testResult.success && testResult.available_models.length > 0 && (
            <div className="mt-1 opacity-70">
              Các mô hình có sẵn: {testResult.available_models.join(", ")}
            </div>
          )}
        </div>
      )}
      ...
      {editing && (
        <div className="mt-2.5 ml-5.5 space-y-2.5 rounded-lg border border-gray-800 bg-gray-950/60 p-3">
          ...
          <div>
            <label htmlFor={`${editPrefix}-apikey`} className="mb-1 block text-xs text-gray-500">API Key（Để trống để giữ lại giá trị hiện tại)</label>
            <input
              ...
              placeholder="Để trống để giữ lại các giá trị hiện có…"
          ...
          {providerId === "gemini-aistudio" && (
            <div>
              ...
              <input
                ...
                placeholder="Sử dụng địa chỉ chính thức theo mặc định…"
                className={inputClsPlaceholder}
              />
            </div>
          )}
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isVertex) {
        ...
        if (!file) {
          setError("Vui lòng chọn một tệp thông tin xác thực");
          setSaving(false);
          return;
        }
        ...
        if (!apiKey.trim()) {
          setError("Vui lòng đầu vào API Key");
          setSaving(false);
          return;
        }
...
export function CredentialList({ providerId, onChanged }: Props) {
  ...
  // Sử dụng ref để lưu trữ onChanged nhằm ổn định tham chiếu làm mới và tránh các vòng lặp vô hạn do kết xuất lại thành phần gốc gây ra.
  ...
  // Sau khi người dùng thao tác: Làm mới danh sách + Thông báo thành phần gốc
    ...
    <div>
      ...
      {credentials.length === 0 && !showAdd && (
        <div className="rounded-lg border border-dashed border-gray-700 px-4 py-6 text-center">
          <p className="text-sm text-gray-500">Chưa có chìa khóa</p>
          <button
            type="button"
            onClick={() => setShowAdd(true)}
          ...
          >
            <Plus className="h-3 w-3" /> Thêmchìa khóa đầu tiên
          </button>
        </div>
      )}
