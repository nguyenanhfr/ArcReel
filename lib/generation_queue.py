...
class GenerationQueue:
    ...
    async def enqueue_task(
        ...
        *,
        ...
        if not result.get("deduped"):
            logger.info("Nhiệm vụ vào hàng đợi task_id=%s type=%s", result["task_id"], task_type)
        else:
            logger.debug("Nhiệm vụ bị trùng lặp task_id=%s", result["task_id"])
        return result
    ...
    async def claim_next_task(self, media_type: str) -> dict[str, Any] | None:
        ...
        if task:
            logger.debug("Nhiệm vụ đã được nhận task_id=%s", task["task_id"])
        return task
    ...
    async def requeue_running_tasks(self, *, limit: int = 1000) -> int:
        ...
        if recovered > 0:
            logger.warning("Thu hồi %d nhiệm vụ đang chạy", recovered)
        return recovered
