...
class ProjectArchiveService:
    ...
    def get_export_diagnostics(
        ...
        *,
        ...
        if not self.project_manager.project_exists(project_name):
            raise FileNotFoundError(f"Dự án '{project_name}' Không tồn tại hoặc chưa được khởi tạo")
        ...
        *,
        ...
        if conflict_policy not in {"prompt", "rename", "overwrite"}:
            raise ProjectArchiveValidationError(
                "không hợp lệChiến lược xung đột",
                errors=[f"conflict_policy Chỉ hỗ trợ prompt, rename hoặc overwrite, nhận được: {conflict_policy}"],
            )
        ...
        try:
            ...
            raise ProjectArchiveValidationError(
                "Tệp tải lên không phải là lưu trữ ZIP hợp lệ",
                errors=[str(exc)],
            ) from exc
    ...
    def _prepare_export_snapshot(
        ...
        *,
        ...
        # Thu thập các mục cấp cao không chuẩn từ thư mục nguồn, ghi lại vào chẩn đoán (ngay cả khi đã bị lọc không xuất)
        excluded_entries = self._collect_pass_through_entries(source_dir)
        for entry in excluded_entries:
            diagnostics.add(
                "warnings",
                "non_standard_entry_excluded",
                f"Thư mục/tệp cấp cao không chuẩn '{entry}' Không bao gồm trong xuất",
                location=entry,
            )
            ...
            for index, episode_meta in enumerate(episodes):
                ...
                if isinstance(script_file, str) and script_file.strip():
                    ...
                    if repaired_script and repaired_script != script_file.replace("\\", "/"):
                        ...
                        diagnostics.add(
                            ...
                            f"{script_location}: Tự động sửa thành {repaired_script}",
                ...
                if not script_path.exists():
                    diagnostics.add(
                        "blocking",
                        "missing_script_file",
                        f"{script_location}: Tệp được tham chiếu không tồn tại: {script_path_rel}",
                        location=script_location,
                    )
                    continue
                ...
                if script_payload is None:
                    diagnostics.add(
                        "blocking",
                        "invalid_script_json",
                        f"Không thể phân tích tệp kịch bản: {script_path_rel}",
                        location=script_location,
                    )
                    continue
        ...
        *,
        ...
        if isinstance(novel, dict) and "source_file" in novel:
            ...
            diagnostics.add(
                ...
                "novel.source_file từĐoạn đã bị loại bỏ, Đã Xóa",
                location=f"{script_path_rel}:novel.source_file",
            )

        # Loại bỏ tập hợp cấp episode đã bị loại bỏ từ đoạn
        for deprecated_field in ("characters_in_episode", "clues_in_episode"):
            if deprecated_field in script_payload:
                ...
                diagnostics.add(
                    ...
                    f"{deprecated_field} từĐoạn đã bị loại bỏ (chuyển sang tính toán khi đọc), Đã Xóa",
        ...
        for index, item in enumerate(raw_items):
            ...
            if clues_field not in item:
                ...
                diagnostics.add(
                    "auto_fixed",
                    "missing_clues_field",
                    f"{items_key}[{index}]: Bổ sung các đoạn bị thiếu {clues_field}",
            ...
            if assets is None:
                ...
                diagnostics.add(
                    ...
                    f"{items_key}[{index}]: Bổ sung các đoạn bị thiếu generated_assets",
                ...
                if missing_keys:
                    ...
                    diagnostics.add(
                        ...
                        (f"{items_key}[{index}].generated_assets: Bổ sung các đoạn mặc định {', '.join(sorted(missing_keys))}"),
                        location=f"{location_prefix}.generated_assets",
                    )
            ...
            if isinstance(characters, list):
                for character_name in characters:
                    ...
                    diagnostics.add(
                        ...
                        f"Tự động bổ sung định nghĩa nhân vật còn thiếu: {character_name}",
            ...
            if isinstance(clues, list):
                ...
                if missing_clues:
                    diagnostics.add(
                        ...
                        (
                            f"{items_key}[{index}]: {clues_field} Trích dẫn cái không tồn tại trong "
                            f"project.json Manh mối: {', '.join(missing_clues)}"
                        ),
                        location=f"{location_prefix}.{clues_field}",
                    )
        ...
        if canonical_path.exists():
            if normalized_value != canonical_rel:
                ...
                diagnostics.add(
                    ...
                    f"{location}: chuẩn hóa thành {canonical_rel}",
                    location=location,
                )
                return True
            return False
    ...
    def _load_json_file(self, path: Path) -> dict[str, Any] | None:
        ...
        try:
            ...
            return None
        logger.warning("Vượt ngoài phạm vi đường dẫn, từ chối đọc: %s", real)
        return None
    ...
    def _validate_scope(scope: str) -> None:
        if scope not in {"full", "current"}:
            raise ValueError(f"scope Chỉ hỗ trợ full hoặc current, nhận được: {scope}")
    ...
    def _scan_archive_members(self, archive: zipfile.ZipFile) -> list[ArchiveMember]:
        ...
        for info in archive.infolist():
            if info.flag_bits & 0x1:
                raise ProjectArchiveValidationError(
                    "Xác thực gói nhập thất bại",
                    errors=[f"ZIP Bao gồm mục được mã hóa, không thể nhập: {info.filename}"],
                )
            ...
            if normalized_name.startswith("/"):
                raise ProjectArchiveValidationError(
                    "Xác thực gói nhập thất bại",
                    errors=[f"ZIP Bao gồm các mục đường dẫn tuyệt đối: {info.filename}"],
                )
            ...
            if any(part == ".." for part in parts):
                raise ProjectArchiveValidationError(
                    "Xác thực gói nhập thất bại",
                    errors=[f"ZIP Bao gồm mục vượt qua đường dẫn: {info.filename}"],
                )
            ...
            if stat.S_ISLNK(mode):
                raise ProjectArchiveValidationError(
                    "Xác thực gói nhập thất bại",
                    errors=[f"ZIP Bao gồm mục liên kết biểu tượng: {info.filename}"],
                )
    ...
    def _locate_project_root(
        ...
        if manifest_members:
            ...
            if len(root_candidates) != 1:
                raise ProjectArchiveValidationError(
                    "Xác thực gói nhập thất bại",
                    errors=["ZIP Chứa nhiều arcreel-export.json, không thể xác định thư mục gốc Dự án"],
                )
            ...
            if not any(member.parts == (*root_parts, self.project_manager.PROJECT_FILE) for member in visible_members):
                raise ProjectArchiveValidationError(
                    "Xác thực gói nhập thất bại",
                    errors=["Gói xuất chính thức dự án.json bị thiếu"],
                )
        ...
        if not root_candidates:
            raise ProjectArchiveValidationError(
                "Xác thực gói nhập thất bại",
                errors=["ZIP Không tìm thấy project.json"],
            )
        if len(root_candidates) != 1:
            raise ProjectArchiveValidationError(
                "Xác thực gói nhập thất bại",
                errors=["ZIP Chứa nhiều project.json, không thể xác định thư mục gốc của dự án"],
            )
    ...
    def _extract_archive_root(
        ...
        for member in members:
            ...
            try:
                ...
                raise ProjectArchiveValidationError(
                    "Xác thực gói nhập thất bại",
                    errors=[f"Đường dẫn giải nén vượt quá giới hạn: {'/'.join(member.parts)}"],
                ) from exc
